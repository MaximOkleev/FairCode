...
class ProblemController(private val problemService: ProblemService) {
    ...
    fun create(
        ...
        if (payload.role != "ADMIN") {
            logger.warn { "Пользователь ${payload.userId} попытался создать задачу (требуется ADMIN)" }
    ...
    fun update(
        ...
        // Только администраторы могут обновлять задачи
        if (payload.role != "ADMIN") {
            logger.warn { "Пользователь ${payload.userId} попытался обновить задачу (требуется ADMIN)" }
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
    ...
    fun delete(
        ...
        if (payload.role != "ADMIN") {
            logger.warn { "Пользователь ${payload.userId} попытался удалить задачу (требуется ADMIN)" }
            return ResponseEntity.status(HttpStatus.FORBIDDEN).build()
        }
